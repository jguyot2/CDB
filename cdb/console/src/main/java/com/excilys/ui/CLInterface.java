...
public class CLInterface {
    ...
    private void createComputerCommand() {
        ...
        if (!strCompanyId.isEmpty()) {
            ...
            Optional<Company> companyOpt = this.companyValidator.findById(companyId);
        ...
        try {
            ...
            if (comp.isPresent()) {
                ...
                if ("1".equals(sc.nextLine().trim())) {
                    int nbdeletedCompany = this.companyValidator.deleteCompanyById(id);
    ...
    private void deleteComputerCommand() {
        ...
        if (this.computerValidator.delete(id) > 0) {
     ...
     */
    private void executeCommand(final CLICommand commandToExecute) {
    ...
    private void getComputerDetailsCommand() {
        ...
        Optional<Computer> computerOpt = this.computerValidator.findById(computerId);
    ...
    private void listCompaniesCommand() {
    ...
    private void listComputersCommand() {
        List<Computer> computerList = this.computerValidator.fetchList();
    ...
    private void updateComputerCommand() {
        ...
        Optional<Computer> optFoundComputer = this.computerValidator.findById(id);
        ...
        int updated;
        try {
            updated = this.computerValidator.update(foundComputer);
        } catch (InvalidComputerException e) {
            System.out.println("Instance incorrecte de Computer crée");
            for (ComputerProblems problem : e.getProblems()) {
                System.out.println(problem.getExplanation());
