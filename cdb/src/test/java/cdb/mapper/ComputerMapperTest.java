...
public class ComputerMapperTest {
    private static final Company[] fakeCompanyList = {
            new Company("POUET", 1), new Company("J'AIME L'OCA", 2),
            new Company("Café Oz", 5), new Company("Chocolatine", 10)
    };

    private static final LocalDate[] localDates = {
            LocalDate.of(1985, 1, 1), LocalDate.of(1985, 1, 19),
    ...
    private static final CompanyDTO[] companyDTOs =
        {
                new CompanyDTO("POUET", "1"), new CompanyDTO("J'AIME L'OCA", "2"),
                new CompanyDTO("Café Oz", "5"), new CompanyDTO("Chocolatine", "10")
        };
    ...
    public void doubleCallTest() {
        for (int i = 0; i < fakeComputerList.length; ++i) {
            assertEquals(
                ComputerMapper.computerToDTO(
                    ComputerMapper.computerDTOToComputer(computerDTOList[i]).get()).get(),
                computerDTOList[i]);
            assertEquals(ComputerMapper
                .computerDTOToComputer(ComputerMapper.computerToDTO(fakeComputerList[i]).get())
                .get(), fakeComputerList[i]);
        }
    }
}
