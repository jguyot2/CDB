...
public class CompanyMapperTest {
    ...
    private final Company[] validCompanies =
        {
                ...
                new Company("Fromage", 543),
                new Company("Camembert"),
                new Company("pouet")
        };

    private final CompanyDTO[] validCompanyDTOs =
        {
                new CompanyDTO("Company1", "1"),
                new CompanyDTO("Company2", "2"),
                new CompanyDTO("Fromage", "543"),
                new CompanyDTO("Camembert", null),
                new CompanyDTO("pouet", "0")
        };
    ...
    public void companyToDTOTest() {
        for (int i = 0; i < validCompanies.length; ++i) {
            ...
            assertTrue(validCompanies[i].getId() == 0
                || Long.toString(validCompanies[i].getId())
                    .equals(validCompanyDTOs[i].getId()));
        }
    ...
    public void doubleCallTest() {
        for (int i = 0; i < validCompanyDTOs.length; ++i) {
            assertEquals(validCompanyDTOs[i],
                CompanyMapper.companyToDTO(
                    CompanyMapper.companyDTOToCompany(validCompanyDTOs[i]).get()).get());
            assertEquals(validCompanies[i],
                CompanyMapper
                    .companyDTOToCompany(CompanyMapper.companyToDTO(validCompanies[i]).get())
                    .get());
            assertEquals(validCompanies[i].getId(),
                CompanyMapper
                    .companyDTOToCompany(CompanyMapper.companyToDTO(validCompanies[i]).get())
                    .get().getId());
        }
