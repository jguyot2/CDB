...
public class ComputerService implements SearchValidator<Computer> {
	 ...
	 */
	@Override
	public List<Computer> fetchList(@Nullable final Page page) {

		try {
			return this.computerSearcher.fetchList(page == null ? new Page() : page);
		} catch (DaoException e) {
	...
	public List<Computer> fetchList(@Nullable final String search) {
		try {
	 ...
	 */
	void setComputerSearcher(final ComputerSearcher newComputerSearcher, final ComputerUpdater newComputerUpdater) {
	 ...
	 */
	public int update(@Nullable final Computer newComputervalue) throws InvalidComputerException {
	...
	public List<Computer> fetchList(@Nullable final List<SortEntry> sortEntries) throws DuplicatedSortEntriesException {
		if (sortEntries == null || sortEntries.isEmpty()) {
